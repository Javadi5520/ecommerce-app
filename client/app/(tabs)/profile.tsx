...
export default function Profile() {
  ...
  return (
    <SafeAreaView className="flex-1 bg-surface" edges={["top"]}>
      ...
      <ScrollView className="flex-4 px-4" contentContainerStyle={!user ? { flex: 1, justifyContent: "center", alignItems: "center" } : { paddingTop: 16 }}>
        {!user ? (
          // Gest User Screen
          <View className="items-center w-full">
            ...
            <Text className="text-primary font-bold text-xl mb-2">Gest User</Text>
          ...
          <>
            ...
            <View className="bg-white rounded-xl border border-gray-100/75 p-2 mb-4">
              {PROFILE_MENU.map((item, index) => (
                <TouchableOpacity
                  key={item.id}
                  className={`flex-row items-center p-4 ${index !== PROFILE_MENU.length - 1 ? "border border-gray-100" : ""}`}
