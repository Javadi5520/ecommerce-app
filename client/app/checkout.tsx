...
export default function Checkout() {
  ...
  const fetchAddress = async () => {
    const addList = dummyAddress;
    if (addList.length > 0) {
      // Find default or first
      const def = addList.find((a: any) => a.isDefault) || addList[0];
  ...
  return (
    <SafeAreaView className="flex-1 bg-surface" edges={["top"]}>
      ...
      <View className="p-4 bg-white shadow-lg border-t border-gray-100">
        ...
        {/* Place Order Button */}
        <TouchableOpacity onPress={() => router.push("/orders")} className={`p-4 rounded-xl items-center ${loading ? "bg-gray-400" : "bg-primary"}`} disabled={loading}>
